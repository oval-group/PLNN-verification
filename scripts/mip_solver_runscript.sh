...
do
    target=$(echo $prop | gawk 'match($0, /(property[0-9]+\/.+)\.rlv/, arr) {print "results/ACAS/MIP/" arr[1] ".txt"}')
    if [ ! -f $target ]; then
...
do
    ...
    target="results/collisionDetection/MIP/$target_fname.txt"
    if [ ! -f $target ]; then
...
do
    ...
    target="results/twinLadder/MIP/$target_fname.txt"
    if [ ! -f $target ]; then
        echo "$INSTR $EXE $MAX_MEM $MAX_TIME $target $METHOD $prop"
        $INSTR $EXE $MAX_MEM $MAX_TIME $target $METHOD $prop
    fi
